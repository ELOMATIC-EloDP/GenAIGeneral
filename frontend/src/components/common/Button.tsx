...
interface sharepointButtonProps extends IButtonProps {
  onClick: () => void;
}

export const SharepointButton: React.FC<ShareButtonProps> = ({onClick}) => {
  const sharepointButtonStyles: ICommandBarStyles & IButtonStyles = {
      root: {
        width: 180,
        height: 32,
        borderRadius: 4,
    ...
    return (
      <CommandBarButton
              styles={sharepointButtonStyles}
              iconProps={{ iconName: 'Share' }}
              onClick={onClick}
              text="Elsa In-House Data"
      />
    )
}